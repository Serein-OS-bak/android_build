...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^serein_") ; then
        SEREIN_BUILD=$(echo -n $1 | sed -e 's/^serein_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $SEREIN_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        SEREIN_BUILD=
    fi
    export SEREIN_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    SEREIN_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/serein/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the SEREIN model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch serein_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the sereinOS GitHub
        T=$(gettop)
        cd $T > /dev/null
        if [[ $NO_ROOMSERVICE == true ]]; then
            echo "Roomservice turned off, type in 'export NO_ROOMSERVICE=false' if you want it back on"
        else
            vendor/serein/build/tools/roomservice.py $product
        fi
        cd - > /dev/null
        check_product $product
    fi
...
function repopick() {
    T=$(gettop)
    $T/vendor/serein/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $SEREIN_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.serein.device | grep -q "$SEREIN_BUILD");
    then
        ...
        echo "The connected device does not appear to be $SEREIN_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.serein.device | grep -q "$SEREIN_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
. $ANDROID_BUILD_TOP/vendor/serein/build/envsetup.sh
